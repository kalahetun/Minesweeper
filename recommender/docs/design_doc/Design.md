...
*   适配工作:
    *   创建一个协调器/主循环 (Coordinator/Main Loop)，负责调用 `Optimizer` 获取 `x_next`，然后调用已有的执行器 (Executor) 的 API 来执行它，最后将结果 `y` 再反馈给 `Optimizer`。
    *   开发一个响应分析器 (Response Analyzer) 模块，负责将执行器返回的原始观测数据（延迟、状态码、Trace）计算成目标函数定义的严重性评分 `y`。
...
*   Sprint 3 (Week 5-6): 响应分析与评分
    ...
    *   实现技术规格中定义的严重性评分函数 `f(x)`。
*   Sprint 4 (Week 7-8): 协调器与端到端测试
    *   编写协调器主循环，将 `Optimizer`、`Executor Client` 和 `Analyzer` 串联起来。
...
1.  定义数据结构: 在 Python 中精确定义 `FaultPlan (x)` 和 `RawObservation` 的数据结构
    *   使用 `dataclass` 定义所有核心类型（`FaultPlan`, `RawObservation`, `AnalyzerConfig`, `BestResult`, `OptimizationProgress`, `SessionState` 等）
    *   包含字段验证（`__post_init__`）、序列化方法（`to_dict`, `from_dict`）
    *   定义枚举类型：`FaultType` (delay/abort/error_injection), `SessionStatus` (PENDING/RUNNING/STOPPING/COMPLETED/FAILED)
    
2.  实现 `Space Converter`: 将用户的 YAML/JSON 配置转换为 scikit-optimize 的 Dimension 对象
    *   支持三种维度类型：categorical (值列表), real (min/max), integer (min/max)
    *   支持条件维度处理（仅当另一个维度满足条件时有效）：3 种策略可选
      - Expand (默认): 为条件组合创建所有维度 → 空间大但简单，适合 Phase 1
      - Filter: 动态过滤无效点 → 紧凑但复杂
      - Encode: 编码为高维连续空间 → 高效但需要解码
    *   配置验证：维度名称唯一、类型检查、条件关系检查
    
3.  实现 `Response Analyzer`: 将原始观测数据量化为严重性评分 $y \in [0, 10]$
    *   Bug 维度 (BugScorer): HTTP 5xx→10.0, 4xx→8.0, ERROR 日志→6.0, error_rate>0→3.0, 否则→0.0
    *   性能维度 (PerformanceScorer): 若 latency > threshold → 10.0，否则 (latency - baseline)/(threshold - baseline) * 9.0
    *   结构维度 (StructureScorer): 通过 Trace 比对检测重试、降级、错误、瓶颈
    *   加权聚合: $y_{total} = \frac{w_{bug} \cdot Score_{Bug} + w_{perf} \cdot Score_{Perf} + w_{struct} \cdot Score_{Struct}}{w_{bug} + w_{perf} + w_{struct}}$
    *   实现 Fail-Safe 原则：计算失败返回默认值，不中断整个循环
    ```
...
| 指标 | 目标值 | 说明 |
|--|-|--|
| 单次优化循环延迟 | < 600ms | propose(20ms) + HTTP(500ms) + analyze(50ms) + record(30ms) |
| 单次 API 响应延迟 | < 100ms | 不包含后台优化线程的执行时间 |
| 并发会话数 | >= 10 | 单节点支持至少 10 个并发优化会话 |
| 模型训练时间 | < 200ms | 每次 `record()` 后重训练随机森林的时间 |
| 内存消耗 | < 500MB | 单个 Session 的平均内存占用 |

### 功能指标
| 指标 | 目标值 | 说明 |
|--|-|--|
| 搜索空间维度 | <= 20 | 故障计划参数的上限 |
| 评分范围 | [0, 10] | 严重性评分的规范化范围 |
| 会话存活时间 | 支持配置 | 默认 24 小时 |
| 故障类型 | 3+ | delay, abort, error_injection |

### 可靠性指标
| 指标 | 目标值 | 说明 |
|--|-|--|
| 优化过程容错性 | Fail-Safe | 缺失 Trace 数据不导致整个循环失败 |
| API 可用性 | >= 99.9% | 在 1 个月内的累计可用时间 |
| 数据一致性 | 强一致性 | SessionManager 使用 Lock 保证 |
...
3.  Response Analyzer (响应分析器):
    *   业务能力: 领域知识量化。
    *   职责:
        *   封装将原始、多模态的系统观测数据（延迟、HTTP 状态码、Trace 结构）转化为单一、标准化的严重性评分 `y` 的所有业务逻辑。
        *   这个模块是注入领域专家知识的地方。
...
好的，我们现在进入最关键的详细设计阶段。基于上一轮的讨论，我们将 BO (Bayesian Optimizer) 系统 分解为具体的、可开发的模块。这个分解将直接映射到您项目中的代码结构（例如，Python 包和类）。
...
*   职责: 封装所有对故障注入执行器控制平面的 HTTP API 调用。
...
*   核心逻辑:
    ...
    *   `apply_and_observe(plan: dict) -> dict:`:
        ...
        3.  轮询或等待一个回调/Webhook，以获取实验的最终观测结果（这部分交互需要与执行器协同设计）。
        4.  返回包含所有原始观测数据的字典。
