...
## Phase 7: 用户故事 5 - 多 Pod 故障分布 (优先级: P2)
...
- [ ] T040 [US5] 验证当前百分比实现在每个 Pod 独立工作（数学证明：独立 30% = 聚合 30%）
- [ ] T041 [US5] 在 `executor/wasm-plugin/src/lib.rs` 添加调试日志记录每次故障注入决策
- [ ] T042 [US5] 创建 `executor/k8s/tests/test-us5-multi-pod.sh` E2E 测试脚本
- [ ] T043 [US5] 执行 E2E 测试：验证所有验收场景
