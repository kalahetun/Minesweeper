...
pub fn execute_fault_with_time_control(
    ...
    match should_inject_fault(&rule_timing, &request_timing) {
        TimeControlDecision::Inject => {
            debug!(
                "Time control decision: Inject - executing fault for context {}",
                context_id
            );
        ...
        TimeControlDecision::Expired => {
            info!(
                "Time control decision: Expired - rule has expired for context {}",
                context_id
            );
            Action::Continue
        }
...
fn execute_abort(
    ...
    let body = abort
        .body
        .as_deref()
        .unwrap_or("Fault injection: Service unavailable");
    let headers = vec![
    ...
    debug!(
        "Sending abort response: status={}, body_len={}",
        abort.http_status,
        body.len()
    );
