...
use executor::{DelayManager, FaultExecutorContext};
...
impl PluginRootContext {
    fn define_metrics(&mut self) {
        ...
        match proxy_wasm::hostcalls::define_metric(
            ...
            Ok(metric_id) => {
                debug!(
                    "Defined delay_duration_milliseconds metric with ID: {}",
                    metric_id
                );
            ...
            Err(e) => {
                warn!(
                    "Failed to define delay_duration_milliseconds metric: {:?}",
                    e
                );
            }
        }
    ...
    fn dispatch_config_request(&mut self) {
        ...
        debug!(
            "Dispatching HTTP call to control plane: {}",
...
impl Context for PluginRootContext {
    fn on_http_call_response(
        ...
        info!(
            "Received HTTP response - Status: {}, Body size: {}",
            response_status, body_size
        );
            ...
            if let Some(body) = self.get_http_call_response_body(0, body_size) {
                if let Ok(body_str) = std::str::from_utf8(&body) {
                    ...
                    match CompiledRuleSet::from_policies_response(&body, identity_ref) {
                        Ok(ruleset) => {
                            info!(
                                "Successfully parsed {} rules from control plane (after filtering)",
                                ruleset.rules.len()
                            );
                                ...
                                *rules = Some(ruleset);
                                ...
                                if let Some(ref rs) = *rules {
                                    for (i, rule) in rs.rules.iter().enumerate() {
                                        debug!(
                                            "Rule {}: {} with {} fault percentage",
                                            i, rule.name, rule.fault.percentage
                                        );
            ...
            if let Ok(config_str) = std::str::from_utf8(&config_bytes) {
                ...
                if config_str.is_empty() {
                    ...
                    if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(config_str) {
                        if let Some(addr) = json_value
                            .get("control_plane_address")
                            .and_then(|v| v.as_str())
                        {
    ...
    fn on_http_call_response(
        ...
        match self.pending_action.take() {
            Some(PendingAction::StartDelay { fault, rule_name }) => {
                ...
                info!(
                    "Start delay completed for context {}, now injecting fault from rule '{}'",
                ...
                if let Some(delay) = &fault.delay {
                    if delay.fixed_delay_ms > 0 {
                        ...
                        if let Some(metric_id) = self.metrics.delay_duration_histogram {
                            if let Err(e) =
                                proxy_wasm::hostcalls::record_metric(metric_id, duration_ms)
                            {
                        ...
                        match self.dispatch_http_call(
                            ...
                            Ok(_) => {
                                info!(
                                    "Delay fault triggered after start_delay for context {} - {}ms",
                ...
                if let Some(abort) = &fault.abort {
                    info!(
                        "Executing abort fault after start_delay: {} for context {}",
                    ...
                    let body = abort
                        .body
                        .clone()
                        .unwrap_or_else(|| "Fault injection: Service unavailable".to_string());
            ...
            Some(PendingAction::DelayFault) => {
                // Delay fault 完成，继续请求
                info!(
                    "Delay fault completed for context {}, resuming request",
                    self.context_id
                );
                self.resume_http_request();
            }
            None => {
                // 没有等待的动作，这应该不会发生
                warn!(
                    "Unexpected http_call_response for context {} with no pending action",
                    self.context_id
                );
                self.resume_http_request();
            }
...
impl HttpContext for PluginHttpContext {
    fn on_http_request_headers(&mut self, _num_headers: usize, _end_of_stream: bool) -> Action {
        ...
        let rules_guard = match self.rules.lock() {
            ...
            Err(e) => {
                warn!(
                    "Failed to acquire lock for rules: {:?}, allowing request to continue",
                    e
                );
        ...
        if let Some(matched_rule) = find_first_match(&request_info, rules) {
            info!(
                "Request matched rule '{}' with {}% fault probability",
            ...
            if matched_rule.fault.duration_seconds > 0 {
                ...
                if rule_age_ms > validity_window_ms {
                    ...
                    debug!(
                        "Rule '{}' is still valid (age: {}ms <= validity: {}ms)",
                        matched_rule.name, rule_age_ms, validity_window_ms
                    );
                }
            }
            ...
            info!(
                "Fault decision: rule='{}', random={}, threshold={}%, inject={}",
            ...
            if matched_rule.fault.start_delay_ms > 0 {
                ...
                info!(
                    "Applying start_delay_ms: {}ms for rule '{}' context {}",
                ...
                match self.dispatch_http_call(
                    ...
                    Ok(_) => {
                        info!(
                            "Start delay initiated for context {} - {}ms",
                            self.context_id, start_delay_ms
                        );
                        return Action::Pause;
                    }
                    Err(e) => {
            ...
            if let Some(delay) = &matched_rule.fault.delay {
                if delay.fixed_delay_ms > 0 {
                    ...
                    match self.dispatch_http_call(
                        ...
                        Err(e) => {
                            warn!(
                                "Failed to dispatch delay call: {:?}, continuing without delay",
                                e
                            );
                            self.pending_action = None;
                            return Action::Continue;
