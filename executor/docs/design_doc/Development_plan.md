...
下面我为你准备了第一阶段 (原型验证 - "让它跑起来") 的全套 Prompt。这些 Prompt 基于你的详细设计文档，旨在引导 AI 生成每个任务模块的核心骨架代码。

### 使用说明

1.  分步进行: 严格按照任务 ID 的顺序，一次向 AI 提供一个 Prompt。不要试图一次性把所有任务都丢给它。
2.  提供上下文: 在开始第一个 Prompt (C-1) 之前，可以先给 AI 一个总体上下文，这将极大地提升后续生成代码的连贯性和准确性。
...
>
> 接下来，我将分步请求你为我生成第一阶段（原型验证） 的代码。请确保你的代码风格简洁、符合 Go 的最佳实践，并遵循我后续提供的具体设计要求。我们的目标是快速搭建一个能跑通核心数据流的原型。
...
>     *   当一个新客户端连接时，`ConfigDistributor` 首先将当前的全量配置发送给这个新客户端。
>     *   然后，`ConfigDistributor` 将这个客户端的连接（例如，一个 `chan string`）注册起来。
>     *   每当 `ConfigDistributor` 因监听到变更而生成了新的全量配置时，它会将这个新配置广播给所有已注册的客户端连接。
...
>     *   实现一个 `impl CompiledRuleSet` 的方法，例如 `from_slice(bytes: &[u8]) -> Result<Self, serde_json::Error>`，该方法在反序列化后，会遍历所有规则并预编译所有正则表达式。
...
>     *   匹配逻辑应遵循短路原则：任何一个条件不匹配，立刻返回 `false`。
>     *   为了性能，应避免在匹配逻辑中进行不必要的字符串分配（多使用 `&str`）。
> 4.  在 `src/lib.rs` 的 `PluginHttpContext` 中，调用 `find_first_match`。
> 5.  为 `matcher.rs` 中的核心匹配逻辑编写详尽的单元测试。使用 `#[cfg(test)]` 模块，模拟 `HttpContext` trait (如果需要) 和各种规则。
...
>     *   在 `CompiledRuleSet::from_slice` 方法中，增加预解析 `fixedDelay` 字符串的逻辑。
> 3.  在 `executor.rs` 中，实现核心执行函数：
>     *   `fn execute_fault(http_context: &dyn HttpContext, fault: &Fault) -> Action`
>     *   该函数首先进行概率检查 (`Percentage`)。可以使用 `proxy_wasm::hostcalls::get_random_bytes` 或一个简单的伪随机数生成器。
>     *   如果概率命中：
...
> 3.  为 `rootCmd` 添加持久化全局标志 (Persistent Flags)：
...
>     *   任何一个条件未定义，则视为匹配成功。
>     *   Path Matcher: 支持 `prefix`, `exact`, `regex`。在配置加载时（`Config Subscriber` 模块）就应该预编译 `regex` 字符串为 `*regexp.Regexp` 对象，并存入 `PathMatcher` 结构体中，以避免在热路径上编译。
...
>         *   Delay 逻辑: 如果 `fault.Delay` 不为 `nil`，调用 `proxy.SetTimer()`。延迟时间需要从 `fault.Delay.FixedDelay` 字符串解析。这个解析应该在配置加载时完成，并缓存为 `time.Duration`，避免在热路径上解析。然后返回 `types.ActionPause`。
...
>     *   关键: 在反序列化后，立即对规则进行预处理：编译所有 `regex` 字符串，解析所有 `delay` 时间字符串，并将结果存回结构体中。
...
>     *   关键：这个 `Deployment` 的 `PodSpec` 应该包含两个容器：
>         *   一个是你的应用容器 (`httpbin`)。
...
开发者文档的目标读者是未来的你、你的同事，或者任何想要理解项目源码、进行二次开发、修复 Bug 或贡献新功能的开发者。
...
> 1.  高级概述:
>     *   用一两段话阐述项目的目标和要解决的问题。
>     *   包含一张高级别的系统上下文图（可以复用或优化 `Design.md` 中的 PlantUML 图），展示用户、CLI、Control Plane、Data Plane (Envoy+Wasm) 之间的关系。
> 2.  核心设计原则:
...
>     *   为每个技术选项提供 1-2 句的选型理由，特别是要解释为什么选择 Rust 而不是 Go 来开发 Wasm 插件（提及性能、内存安全、TinyGo 的问题等）。
> 4.  核心数据流:
>     *   包含一张数据流图（可以复用 `Design.md` 中的），并配以文字逐步解释从 `hfi-cli policy apply` 到 Wasm 插件注入故障的完整流程。
...
> 要求:
> 1.  模块架构:
>     *   包含一张 Control Plane 的模块架构图（复用 `Design_1_Control_Plane.md` 中的图）。
>     *   详细描述每个核心模块的职责：
...
>     *   包含一张模块架构图（复用 `Design_2_Wasm_plugin.md` 中的图）。
> 2.  模块详解:
...
>     *   使用表格或列表，详细描述每一个字段的含义、类型、是否必需以及可选值。例如：
...
1.  技术风险: Wasm for Envoy 开发有学习曲线。
    *   缓解措施: 第一阶段专门用于技术原型验证，尽早暴露问题。保持 Wasm 插件逻辑简单，复杂性上移到 Control Plane。
2.  进度风险: 任务估算可能不准。
    *   缓解措施: 采用小步快跑的方式，每个阶段都有明确的可交付成果。如果某个任务延期，可以及时调整后续计划，优先保证 MVP 的核心功能。
3.  环境风险: Kubernetes 和 Envoy 的环境配置复杂。
    *   缓解措施: 第一阶段就搭建可复用的本地开发环境 (`docker-compose` 或 `kind`)，并将其代码化，所有开发者使用同一套环境。
...
## 第四阶段（改进）：统一 Policy API 的时间控制 - "支持持久化和临时场景"

目标: 通过为 Policy API 添加 `duration_seconds` 字段，使其能够支持持久化和临时两种故障注入场景。无需额外的 Fault API，一套 API 解决所有需求。
特点: 统一的 Policy API 设计，通过 `duration_seconds` 控制生命周期；自动过期机制简化了临时实验的管理。
预计时间: 3 周（相比原来的 4 周，改进后更加简洁高效）

| 任务 ID | 任务模块        | 任务描述                                                                                                      | 关键技术点/产出                                                                      | 依赖任务 |
|:--------|:----------------|:--------------------------------------------------------------------------------------------------------------|:-----------------------------------------------------------------------------------| :-----  |
| TF-0    | Control Plane   | 升级 Policy 数据模型: 为 `FaultAction` 添加 `start_delay_ms` 和 `duration_seconds` 字段。更新 Validator 和 CLI examples。 | `types/policy.go` 扩展, YAML 验证逻辑升级, 向后兼容性                                | C-4     |
| TF-1    | Control Plane   | 实现 Policy 自动过期机制: 在 `PolicyService` 中实现自动过期定时器，当 `duration_seconds > 0` 时自动删除 Policy。 | 定时器管理, goroutine 生命周期, context 控制                                         | TF-0    |
| TF-2    | Wasm Plugin     | 增强配置模型: 扩展 `CompiledRule` 以支持 `start_delay_ms` 和 `duration_seconds` 字段。                        | `config.rs` 扩展, 反序列化, 字段兼容性处理                                           | W-5     |
| TF-3    | Wasm Plugin     | 实现时间控制逻辑: 实现 `should_inject_fault()` 的时间检查，包括 `start_delay_ms` 和 `duration_seconds` 的有效期检查。 | 时间戳精度, 时间比较函数, 请求生命周期跟踪                                          | TF-2    |
| TF-4    | Wasm Plugin     | 实现指标收集: 在 Wasm 插件中收集故障注入指标，包括匹配计数、阻断计数、延迟统计、错误率等。                        | 原子操作, 计数器管理, 指标聚合                                                        | TF-3    |
| INT-1   | 集成测试       | 单元测试: 为 Policy 新字段、Service 自动过期、Wasm 时间控制逻辑编写单元测试。                                 | 覆盖率 ≥90%, 时间控制的各种边界情况测试                                              | TF-4    |
| INT-2   | 集成测试       | 集成测试: 测试 Control Plane ↔ Wasm Plugin 的完整流程。验证持久化和临时 Policy 的生命周期。                   | 端到端测试脚本, 生命周期验证测试用例                                                  | INT-1   |
| INT-3   | 集成测试       | 性能和时间精度测试: 验证高并发下的指标收集、延迟注入的准确性、自动过期的精确性。                                 | 性能报告, 时间精度基准 (<100ms 偏差)                                                  | INT-2   |
| DOC-3   | 文档           | 补充设计文档: 更新 `Design.md` 的生命周期管理部分。补充 `UNIFIED_TIME_CONTROL_DESIGN.md` 的实现细节。           | 设计文档完善, 统一时间控制设计文档, 最佳实践指南                                      | TF-3    |
| CLI-3   | CLI            | 增强 CLI: 为 Policy 时间字段提供 CLI 支持（如 `--start-delay-ms`, `--duration-seconds`）。                    | Cobra 子命令扩展, 参数验证                                                            | TF-1    |

第四阶段可交付成果:
*   升级的 Policy 数据模型，支持 `start_delay_ms` 和 `duration_seconds` 时间控制字段。
*   自动过期机制：Control Plane 在 Policy 创建时，如果设置了 `duration_seconds`，会自动启动定时器在指定时间后删除该 Policy。
*   增强的 Wasm 插件，支持统一的 `start_delay_ms` 延迟注入逻辑和 `duration_seconds` 有效期检查。
*   精确的指标收集机制（毫秒级精度）。
*   完整的测试覆盖：单元、集成、性能测试（包括时间精度测试）。
*   升级的 CLI，支持新的时间控制参数。
*   完善的设计文档，详细说明统一的时间控制设计和最佳实践。

---

### 任务 TF-0: Control Plane - 升级 Policy 数据模型

**任务概述**: 为 `FaultAction` 结构体添加 `start_delay_ms` 和 `duration_seconds` 字段，支持细粒度的故障时机控制和生命周期管理。

**具体步骤**:
1. 在 `cli/types/policy.go` 中的 `FaultAction` 结构体添加两个新字段
2. 更新 `control-plane/service/validator.go` 中的验证逻辑
3. 更新所有示例文件 `cli/examples/*.yaml` 以展示新字段的使用
4. 确保向后兼容性（新字段使用 `omitempty` 标签）

**依赖**: C-4 | **工作量**: 3-4 小时

---

### 任务 TF-1: Control Plane - 实现 Policy 自动过期机制

**任务概述**: 在 `PolicyService` 中实现自动过期定时器，当 `duration_seconds > 0` 时，在指定时间后自动删除该 Policy。

**具体步骤**:
1. 在 `CreateOrUpdate` 方法中添加定时器逻辑
2. 使用 Go 的 `time.Timer` 或 `time.AfterFunc`
3. 正确处理 context 生命周期，确保 goroutine 不会泄漏
4. 添加日志记录定时器触发和删除事件
5. 编写单元测试验证自动过期逻辑
...
### 任务 TF-2: Wasm Plugin - 增强配置模型

**任务概述**: 扩展 `CompiledRule` 结构，支持 `start_delay_ms` 和 `duration_seconds` 字段的接收和反序列化。

**具体步骤**:
1. 在 `src/config.rs` 的 `CompiledRule` 结构体中添加两个新字段
2. 更新序列化/反序列化逻辑
3. 添加字段有效性检查
4. 编写单元测试验证字段的正确传递
...
### 任务 TF-3: Wasm Plugin - 实现时间控制逻辑

**任务概述**: 在 Wasm 插件中实现 `should_inject_fault()` 的时间检查逻辑，支持 `start_delay_ms` 延迟和 `duration_seconds` 有效期检查。

**具体步骤**:
1. 实现请求到达后的延迟跟踪（`start_delay_ms`）
2. 实现 Policy 有效期检查（`duration_seconds`）
3. 确保时间戳的精度（毫秒级）
4. 处理边界情况：延迟超过请求生命周期等
5. 编写详尽的单元测试，包括各种时间场景

**依赖**: TF-2 | **工作量**: 5-6 小时

---

### 任务 TF-4: Wasm Plugin - 实现指标收集

**任务概述**: 在 Wasm 插件中收集故障注入指标，包括匹配计数、阻断计数、延迟统计等。
...
**任务概述**: 为所有新增功能编写单元测试，确保覆盖率 ≥90%。

**测试覆盖**:
- Policy 数据模型：字段序列化、验证、向后兼容性
- 自动过期机制：定时器触发、context 生命周期、goroutine 泄漏
- Wasm 配置：字段传递、反序列化
- 时间控制逻辑：各种时间场景、边界情况
- 指标收集：原子操作、数据一致性
...
**任务概述**: 测试 Control Plane 与 Wasm Plugin 的完整流程，验证持久化和临时 Policy 的生命周期。

**测试场景**:
- 创建持久化 Policy（`duration_seconds = 0`），验证一直有效
- 创建临时 Policy（`duration_seconds > 0`），验证自动删除
- 延迟注入时机（`start_delay_ms`），验证故障在正确时刻执行
- 并发创建多个 Policy，验证独立的生命周期管理
...
**任务概述**: 验证系统在高并发下的指标收集精度、延迟注入的准确性、自动过期的精确性。

**测试内容**:
- 高并发下（1000+ 并发）的指标收集准确性
- 延迟注入的时间精度（目标：< 100ms 偏差）
- 自动过期的精确性（目标：< 1 秒偏差）
- 内存使用情况和泄漏检测
...
**任务概述**: 更新和补充设计文档，详细说明统一时间控制的设计和实现。

**文档更新**:
- 更新 `Design.md` 的 Policy 生命周期管理部分
- 补充 `UNIFIED_TIME_CONTROL_DESIGN.md` 的实现细节和代码示例
- 添加时间控制的最佳实践指南
...
**任务概述**: 为 Policy CLI 添加时间控制参数的支持。

**具体步骤**:
1. 在现有 `policy apply` 命令中添加 `--start-delay-ms` 参数
2. 添加 `--duration-seconds` 参数
3. 更新命令帮助文档和示例
4. 编写集成测试验证参数传递的正确性
...
**按里程碑的时间分布**：
- **Week 1**: TF-0, TF-1, TF-2, TF-3 + 并行 INT-1 开始
- **Week 2**: TF-4, INT-1 完成, INT-2 开始 + DOC-3, CLI-3 并行
- **Week 3**: INT-2 完成, INT-3 开始，DOC-3, CLI-3 收尾和集成

**关键路径**: TF-0 → TF-1 → TF-2 → TF-3 → INT-2 → INT-3

**总耗时**: 约 3 周（相比原来的 4 周减少 25%，因为省去了独立 Fault API 的开发）
...
| 第四阶段（新增） | - | 3.5周 | 临时故障注入API，包括数据模型、业务逻辑、API实现、Wasm扩展、指标收集、完整测试 |
| 总计（前四阶段） | 6周 | 11.5周 | 支持完整的临时故障注入功能，包括per-request start_delay_ms |
